...
export const docMessages: LocaleMessages = {
  'zh-CN': {
    demo: {
      ...
      uploadDesignDecoupleDesc: 'fetch、axios、ky、wx.uploadFile——api 是你的函数，爱怎么发怎么发。',
      uploadDesignTokenTitle: '不绕过基础设施',
      uploadDesignTokenDesc: 'request 实例的拦截器、token 刷新、统一错误处理、loading 态全部照常工作。',
      uploadDesignOssTitle: '兼容任意上传流程',
      uploadDesignOssDesc: 'OSS 签名、COS 鉴权、回调通知——一个异步函数全搞定。传统的 action + data 根本表达不了。',
      uploadDesignTestTitle: '天然可测',
      uploadDesignTestDesc: '传个 async () => ({ url: "/fake" })，零 mock。上传逻辑和 UI 彻底分离，测试只测 UI 行为。',
  ...
  'en-US': {
    demo: {
      ...
      uploadDesignDecoupleDesc: `fetch, axios, ky, wx.uploadFile — the api function is yours; call it however you want.`,
      uploadDesignTokenTitle: `Infrastructure intact`,
      uploadDesignTokenDesc: `Your request instance — interceptors, token refresh, error handling, loading state — all work as usual.`,
      uploadDesignOssTitle: `Any upload flow`,
      uploadDesignOssDesc: `OSS signatures, COS auth headers, callbacks — one async function handles it all. Traditional action + data simply cannot express this.`,
      uploadDesignTestTitle: `Naturally testable`,
      uploadDesignTestDesc: `Pass an async () => ({ url: "/fake" }) — zero mocks. Upload logic and UI are fully separated; tests only verify UI behavior.`,
