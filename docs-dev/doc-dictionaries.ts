...
export const docMessages: LocaleMessages = {
  'zh-CN': {
    demo: {
      ...
      loadFailed: '加载失败',
      uploadDesignDecoupleTitle: '解耦 HTTP 库',
      uploadDesignDecoupleDesc: 'fetch、axios、ky 还是 wx.uploadFile？Upload 不关心。你传一个 api 函数，里面怎么发请求完全由你控制。',
      uploadDesignTokenTitle: 'Token / 拦截器',
      uploadDesignTokenDesc: '项目中已封装好的 request 实例（统一拦截、错误处理、loading 态）直接传给 api 即可，Upload 不绕过你的基础设施。',
      uploadDesignOssTitle: 'OSS 直传',
      uploadDesignOssDesc: '阿里云 / AWS S3 需要先获取签名、拼表单、有时还要回调通知。传统的 action + data 模型根本无法表达这个流程。',
      uploadDesignTestTitle: '测试友好',
      uploadDesignTestDesc: '传一个 async () => ({ url: "..." }) 就能跑通测试，无需 mock 任何 HTTP 库。',
  ...
  'en-US': {
    demo: {
      ...
      uploadDesignDecoupleTitle: `Decouple HTTP Library`,
      uploadDesignDecoupleDesc: `fetch, axios, ky, or wx.uploadFile? Upload does not care. You provide an api function, and how the request is sent is entirely up to you.`,
      uploadDesignTokenTitle: `Token / Interceptor`,
      uploadDesignTokenDesc: `Your already-configured request instance (with unified interceptors, error handling, loading state) can be passed directly to api. Upload does not bypass your infrastructure.`,
      uploadDesignOssTitle: `Direct OSS Upload`,
      uploadDesignOssDesc: `Alibaba Cloud / AWS S3 requires obtaining signatures, assembling forms, and sometimes callback notifications. The traditional action + data model simply cannot express this flow.`,
      uploadDesignTestTitle: `Test-Friendly`,
      uploadDesignTestDesc: `Pass an async () => ({ url: "..." }) and your tests work, no need to mock any HTTP library.`,
