...
export const docMessages: LocaleMessages = {
  'zh-CN': {
    demo: {
      ...
      loadFailed: '加载失败',
      uploadDesignDecoupleTitle: '绑死 HTTP 库',
      uploadDesignDecoupleDesc: 'fetch、axios、ky、wx.uploadFile——你用什么跟 Upload 无关。api 是你的函数，爱怎么发怎么发。',
      uploadDesignTokenTitle: '绕过全局拦截器',
      uploadDesignTokenDesc: '你封装好的 request 实例——拦截器、token 刷新、统一错误处理、loading 态——全部照常工作。Upload 不替你发请求，所以不会绕过任何一层。',
      uploadDesignOssTitle: '搞不定 OSS / COS',
      uploadDesignOssDesc: 'OSS 直传要先拿签名再拼表单，COS 要算鉴权头，有些接口还要回调通知——传统的 action + data 根本表达不了。但一个异步函数可以。',
      uploadDesignTestTitle: '不好写测试',
      uploadDesignTestDesc: '传个 async () => ({ url: "/fake" })，零 mock 零拦截。上传逻辑和 UI 彻底分离，测试只测 UI 行为。',
  ...
  'en-US': {
    demo: {
      ...
      uploadDesignDecoupleTitle: `Locked to one HTTP library`,
      uploadDesignDecoupleDesc: `fetch, axios, ky, wx.uploadFile — doesn't matter to Upload. The api function is yours; call it however you want.`,
      uploadDesignTokenTitle: `Bypasses global interceptors`,
      uploadDesignTokenDesc: `Your configured request instance — interceptors, token refresh, unified error handling, loading state — all work as usual. Upload doesn't send requests on your behalf, so nothing is bypassed.`,
      uploadDesignOssTitle: `Can't handle OSS / COS`,
      uploadDesignOssDesc: `OSS direct upload needs a signature first, then form assembly. COS requires auth headers. Some endpoints need callbacks. Traditional action + data simply cannot express this. But an async function can.`,
      uploadDesignTestTitle: `Hard to test`,
      uploadDesignTestDesc: `Pass an async () => ({ url: "/fake" }) — zero mocks, zero interceptors. Upload logic and UI are fully separated; tests only verify UI behavior.`,
