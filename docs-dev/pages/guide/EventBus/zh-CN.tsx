export default {
  eventbus: {
    ...
    warning: '本库的 EventBus 定位为拦截与切面，而非通用消息通道。它专为埋点遥测、审计日志、AOP 拦截等横切关注点设计，提供一个统一的观测入口。我们不建议用它来做组件间通信、状态同步或事件驱动的业务流转——这些场景请走 props、回调等显式契约。EventBus 在这里的角色是旁路观察者：静默记录发生的一切，不参与、不改变业务逻辑的执行路径。',
    demoTitle: '实时事件演示',
    demoDesc: '点击按钮或输入文字，事件日志实时显示在下方：',
    demoHint: '点击按钮或输入文字...',
    quickTitle: '快速开始',
    quickDesc: '在应用入口调用一次 setEventBusHandler，无需 Provider、无额外依赖。未注册时 emitEvent 仅做一次 null 检查，零运行时开销。',
    structTitle: '事件结构',
    structTh1: '字段', structTh2: '类型', structTh3: '说明',
    structComp: '触发事件的组件名，如 Picker、Upload',
    structType: '事件类别：change click confirm cancel clear delete submit success error refresh select show',
    ...
    eventsTitle: '可用事件一览',
    eventsDesc: '下表中的 payload 是 event.payload 的类型。每一条事件同时携带 props（组件 props 快照）和 timestamp。',
    eventsTh1: '组件', eventsTh2: '事件类型',
    customTitle: '自定义组件',
    customDesc: 'emitEvent 已作为公开 API 导出。你自己的组件也能按 EventBusEvent 签名把结构化事件推送到同一个全局总线。',
    notesTitle: '注意事项',
