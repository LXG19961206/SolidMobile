      ...
      <div style={CARD.wrapper}>
        ...
        <div style={{ padding: '0 16px 12px', 'font-size': '0.8rem', 'line-height': 1.8, color: 'var(--sc-color-text-secondary, #6b7280)' }}>
          ...
          <ul style={{ margin: '0 0 10px', 'padding-left': '1.5rem', 'line-height': 2 }}>
            {isEn() ? (
              <><li>Is method always POST?</li><li>Binary, form-data, or other encoding?</li><li>Extra headers (Authorization, Content-Type)?</li><li>BaseURL? Timeout? Retry strategy?</li><li>Error handling? Progress feedback?</li></>
            ) : (
              <><li>method 一定是 POST 吗？</li><li>binary 还是 form-data，或是其他编码格式？</li><li>额外的请求头（Authorization、Content-Type 等）如何传递？</li><li>BaseURL 从哪来？超时、重试策略如何配置？</li><li>上传异常如何处理？进度如何反馈？</li></>
            )}
          </ul>
          <p style={{ margin: '0 0 10px' }}>
            {isEn() ? (
              <>Turn all of these into props? Then your global HTTP infrastructure — interceptors, middleware, token refresh — is bypassed. A handful of props can never fully describe an HTTP request, and they'd be redundant with your existing setup anyway. Therefore Upload <strong>does not assemble and send upload requests on your behalf</strong> — it provides no action, headers, or data props. The project's global HTTP configuration (interceptors, token refresh, unified error handling) remains fully utilized. <strong>HTTP requests belong to the business layer, not the component layer.</strong></>
            ) : (
              <>若全部暴露为 Props，则每次使用 Upload 都将绕开全局的 HTTP 基础设施。Upload <strong>不代为拼凑发起上传请求</strong>，不提供 action、headers、data 等属性。上传请求由开发者自行发起，组件库不介入这一过程，项目的全局 HTTP 配置（拦截器、Token 刷新、统一错误处理）得以充分利用。<strong>请求应归属业务层，而非组件层。</strong></>
            )}
          </p>
