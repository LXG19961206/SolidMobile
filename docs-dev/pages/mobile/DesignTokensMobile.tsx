...
export const DesignTokensMobile: Component<{ components?: ComponentEntry[]; onNavigate?: (key: string) => void }> = (props) => {
  ...
  const renderColorRow = (label: string, lightColor: string, darkColor: string) => (
    <div style={swatchRow}>
      ...
      <span style={{ 'font-size': '0.8rem', color: '#374151', flex: 1 }}>{label}</span>
      <code style={{ 'font-size': '0.7rem', color: '#9ca3af' }}>{lightColor}</code>
    </div>
  );
    ...
    <MobilePreview title="视觉规范" components={props.components} onNavigate={props.onNavigate}>
      ...
      {semanticGroups.map((group) => (
        <div style={CARD.wrapper}>
          ...
          <div style={CARD.desc}>
            <span style={{ display: 'inline-block', width: '32px', 'text-align': 'center', 'font-size': '0.65rem', color: '#9ca3af' }}>L</span>
            <span style={{ display: 'inline-block', width: '32px', 'text-align': 'center', 'font-size': '0.65rem', color: '#9ca3af', 'margin-left': '10px' }}>D</span>
        ...
        <div style={CARD.body}>
          ...
          {(['xs', 'sm', 'md', 'lg', 'xl', 'xxl'] as const).map((s) => (
            <div style={{ display: 'flex', 'align-items': 'center', gap: '12px', padding: '6px 0', 'border-bottom': '1px solid #f3f4f6' }}>
              <span style={{ 'font-size': '0.75rem', color: '#9ca3af', 'min-width': '24px' }}>{s}</span>
              <span style={{ 'font-size': config.typography.fontSize[s], color: '#374151' }}>
                {config.typography.fontSize[s]}
              </span>
            </div>
          ))}
          <div style={{ 'font-size': '0.85rem', color: '#374151', 'margin': '16px 0 8px' }}>
            <strong>字重 Font Weight</strong>
          </div>
          {(['normal', 'medium', 'semibold', 'bold'] as const).map((w) => (
            <div style={{ display: 'flex', 'align-items': 'center', gap: '12px', padding: '6px 0', 'border-bottom': '1px solid #f3f4f6' }}>
              <span style={{ 'font-size': '0.75rem', color: '#9ca3af', 'min-width': '36px' }}>{w}</span>
              <span style={{ 'font-weight': config.typography.fontWeight[w], 'font-size': '0.95rem', color: '#374151' }}>
        ...
        <div style={CARD.body}>
          {(['sm', 'md', 'lg', 'full'] as const).map((r) => (
            <div style={{ display: 'flex', 'align-items': 'center', gap: '12px', padding: '8px 0' }}>
              ...
              <span style={{ 'font-size': '0.8rem', color: '#374151' }}>{r}</span>
              <code style={{ 'font-size': '0.75rem', color: '#9ca3af' }}>{config.borderRadius[r]}</code>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={CARD.wrapper}>
        ...
        <div style={CARD.body}>
          {(['tight', 'normal', 'relaxed'] as const).map((lh) => (
            <div style={{ display: 'flex', 'align-items': 'center', gap: '12px', padding: '6px 0', 'border-bottom': '1px solid #f3f4f6' }}>
              <span style={{ 'font-size': '0.75rem', color: '#9ca3af', 'min-width': '40px' }}>{lh}</span>
              <span style={{ 'font-size': '0.85rem', color: '#374151' }}>{config.typography.lineHeight[lh]}</span>
              <div style={{
                flex: 1, background: '#f3f4f6', padding: '4px 8px', 'border-radius': '4px',
                'line-height': config.typography.lineHeight[lh], 'font-size': '0.7rem', color: '#6b7280',
              }}>
