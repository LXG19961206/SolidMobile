    ...
    <MobilePreview title="Icon 图标" props={propsData} components={props.components} onNavigate={props.onNavigate}>
      ...
      <div style={CARD.wrapper}>
        ...
        <div style={CARD.body}>
          <div style={{ display: 'flex' as const, 'flex-direction': 'column' as const, 'align-items': 'center' as const, gap: '4px' }}>
            ...
            <span style={{ 'font-size': '0.7rem', color: '#9ca3af' }}>line</span>
          </div>
          <div style={{ display: 'flex' as const, 'flex-direction': 'column' as const, 'align-items': 'center' as const, gap: '4px' }}>
      ...
      <div style={CARD.wrapper}>
        ...
        <div style={{ padding: '0 16px 12px', display: 'flex' as const, gap: '8px' }}>
          <button
            style={{ padding: '6px 12px', border: '1px solid #e5e7eb', 'border-radius': '6px', background: variant() === 'line' ? '#1677ff' : '#fff', color: variant() === 'line' ? '#fff' : '#374151', cursor: 'pointer', 'font-size': '0.8rem' }}
            onClick={() => setVariant('line')}
          >线性</button>
          <button
            style={{ padding: '6px 12px', border: '1px solid #e5e7eb', 'border-radius': '6px', background: variant() === 'fill' ? '#1677ff' : '#fff', color: variant() === 'fill' ? '#fff' : '#374151', cursor: 'pointer', 'font-size': '0.8rem' }}
            onClick={() => setVariant('fill')}
          >填充</button>
          <input
            style={{ flex: 1, padding: '6px 12px', border: '1px solid #e5e7eb', 'border-radius': '6px', 'font-size': '0.8rem' }}
            placeholder="搜索图标..."
            value={search()}
        ...
        <div style={{ padding: '0 16px 16px', display: 'flex' as const, 'flex-wrap': 'wrap' as const, gap: '8px' }}>
          <For each={filteredIcons()}>
            {(name) => (
              <div
                style={{ display: 'flex' as const, 'flex-direction': 'column' as const, 'align-items': 'center' as const, gap: '4px', padding: '8px', border: '1px solid #f3f4f6', 'border-radius': '8px', cursor: 'pointer', 'min-width': '56px' }}
                onClick={() => handleCopy(name)}
              >
                <Icon name={name} variant={variant()} size={22} />
                <span style={{ 'font-size': '0.6rem', color: '#9ca3af', 'max-width': '56px', overflow: 'hidden', 'text-overflow': 'ellipsis', 'white-space': 'nowrap' }}>{name}</span>
              </div>
            )}
          </For>
        </div>
