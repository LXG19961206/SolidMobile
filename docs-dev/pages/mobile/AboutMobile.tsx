...
const LINK = { color: 'var(--sc-color-primary, #1677ff)' };
...
export const AboutMobile: Component<AboutMobileProps> = (props) => {
  ...
  return (
    <MobilePreview title={isEn() ? 'About the Project' : '关于项目'} components={props.components} onNavigate={props.onNavigate}>
      <div style={{ padding: '12px 0' }}>

        {/* Origins */}
        <div style={CARD.wrapper}>
          <div style={CARD.title}>{isEn() ? 'Origins' : '缘起'}</div>
          <div style={CARD.body}>
            ...
            <p style={{ margin: '0' }}>
              {isEn() ? (
                <>The author makes no claim to being a UI designer — the color palette, spacing, and overall visual language draw significant inspiration from{" "}
                  <a href="https://vant-ui.github.io/" target="_blank" rel="noopener" style={LINK}>Vant</a>
        ...
        {/* The First Version */}
        <div style={CARD.wrapper}>
          <div style={CARD.title}>{isEn() ? 'The First Version' : '第一版'}</div>
          <div style={CARD.body}>
            <p style={{ margin: '0' }}>
              {isEn() ? (
        ...
        {/* Renaissance */}
        <div style={CARD.wrapper}>
          <div style={CARD.title}>{isEn() ? 'Renaissance' : '重构'}</div>
          <div style={CARD.body}>
            <p style={{ margin: '0' }}>
              {isEn() ? (
        ...
        {/* Current Status */}
        <div style={CARD.wrapper}>
          <div style={CARD.title}>{isEn() ? 'Current Status' : '当前进度'}</div>
          <div style={CARD.body}>
            <p style={{ margin: '0 0 0.75rem' }}>
              {isEn() ? (
            ...
            <div style={{ display: 'flex', 'flex-wrap': 'wrap', gap: '6px' }}>
              {([
                ...
                <div style={{
                  ...
                  'border-radius': '8px', padding: '8px 10px', 'min-width': '140px', flex: '1 1 auto',
                }}>
                  <div style={{ 'font-size': '0.65rem', 'font-weight': 600, color: 'var(--sc-doc-card-muted, #9ca3af)', 'text-transform': 'uppercase', 'letter-spacing': '0.05em', 'margin-bottom': '6px' }}>
        ...
        {/* Roadmap */}
        <div style={CARD.wrapper}>
          <div style={CARD.title}>{isEn() ? 'Roadmap' : '后续规划'}</div>
          <div style={CARD.body}>
            <p style={{ margin: '0 0 0.75rem' }}>
              {isEn() ? (
        ...
        {/* Acknowledgments */}
        <div style={CARD.wrapper}>
          <div style={CARD.title}>{isEn() ? 'Acknowledgments' : '致谢'}</div>
          <div style={CARD.body}>
            <p style={{ margin: '0' }}>
              {isEn() ? (
                <>This is a solo project, built across evenings and weekends. It is also — in the spirit of full disclosure — a collaboration between one human and one AI: the author and Claude together contributed the vast majority of the code, docs, and design decisions in this repository.{" "}
                  <a href="https://github.com/LXG19961206/SolidMobile" target="_blank" rel="noopener" style={LINK}>GitHub</a>
                  {" "}is always open. Stars, issues, and pull requests are genuinely appreciated.</>
              ) : (
                <>这是一个个人项目，利用业余时间完成。坦诚地讲，这也是一个人与一个 AI 的合作项目——作者与 Claude 共同贡献了本仓库中绝大多数的代码、文档与设计决策。{" "}
                  <a href="https://github.com/LXG19961206/SolidMobile" target="_blank" rel="noopener" style={LINK}>GitHub</a>
                  {" "}始终开放。每一个 star、每一条 issue、每一个 PR，都发自内心地感谢。</>
              )}
            </p>
          </div>
        </div>
