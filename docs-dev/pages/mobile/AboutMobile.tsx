  ...
  return (
    <MobilePreview title={t('about.title')} components={props.components} onNavigate={props.onNavigate}>
      <div style={{ padding: '12px' }}>
        ...
        <div style={CARD.wrapper}><div style={CARD.title}>{t('about.status')}</div><div style={CARD.body}><p style={{ margin: '0 0 0.75rem' }}>{t('about.statusP1')}<strong>{componentCount}</strong>{t('about.statusP1After')}</p><div style={{ display: 'flex', 'flex-wrap': 'wrap', gap: '6px' }}>
          {([
        ...
        <div style={CARD.wrapper}><div style={CARD.title}>{t('about.roadmap')}</div><div style={CARD.body}><p style={{ margin: '0 0 0.75rem' }}>{t('about.roadmapP')}</p><div style={{ display: 'flex', 'flex-wrap': 'wrap', gap: '6px' }}>{([] as string[]).map(name => (<span style={{ padding: '4px 10px', 'font-size': '0.8rem', background: '#fef3c7', 'border-radius': '4px', 'font-family': 'monospace', color: '#92400e' }}>{name}</span>))}</div></div></div>
