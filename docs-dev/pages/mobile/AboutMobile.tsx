  ...
  return (
    <MobilePreview title={isEn() ? 'About the Project' : '关于项目'} components={props.components} onNavigate={props.onNavigate}>
      <div style={{ padding: '12px 0' }}>
        ...
        <h2 style={H2}>{isEn() ? 'Origins' : '缘起'}</h2>
        <p style={P}>
          {isEn() ? (
        ...
        <h2 style={H2}>{isEn() ? 'The First Version' : '第一版'}</h2>
        <p style={P}>
          {isEn() ? (
        ...
        <h2 style={H2}>{isEn() ? 'Renaissance' : '重构'}</h2>
        <p style={P}>
          {isEn() ? (
        ...
        <h2 style={H2}>{isEn() ? 'Current Status' : '当前进度'}</h2>
        <p style={P}>
          {isEn() ? (
        ...
        {/* Component grid grouped by category — mobile-adapted layout */}
        ...
        <h2 style={H2}>{isEn() ? 'Roadmap' : '后续规划'}</h2>
        <p style={P}>
          {isEn() ? (
        ...
        <h2 style={H2}>{isEn() ? 'Acknowledgments' : '致谢'}</h2>
        <p style={P}>
          {isEn() ? (
