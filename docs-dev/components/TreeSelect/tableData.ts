...
export function useTreeSelectTableData() {
  ...
  const propsTables: TableSection[] = [{
    rows: [
      ...
      { name: 'renderItem', type: '(node, selected, expand, toggle) => JSX.Element', def: '—', desc: 'treeselect.props.renderItem' },
      { name: 'onSearch', type: '(keyword, options) => Promise<TreeSelectOption[]> | TreeSelectOption[]', def: '—', desc: 'treeselect.props.onSearch' },
      ...
      { name: 'onUpdateShow', type: '(show: boolean) => void', def: '—', desc: 'treeselect.props.onUpdateShow' },
      ...
      { name: 'maxHeight', type: 'number | string', def: "'80vh'", desc: 'treeselect.props.maxHeight' },
