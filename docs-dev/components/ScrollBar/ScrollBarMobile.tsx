      ...
      <div style={{ padding: '12px', display: 'flex', 'flex-direction': 'column', gap: '12px' }}>
        <Card title={t('scrollbar.demo.basic')} subtitle={t('scrollbar.demoDesc.basic')}>
          <ScrollBar height={200}>
            <div style={{ border: '1px solid var(--sc-color-border, #e5e7eb)', 'border-radius': '8px', padding: '8px', 'font-size': '0.85rem', 'line-height': 2.5, color: '#6b7280' }}>
              {Array.from({ length: 30 }, (_, i) => <div>Line {i + 1} — long content to demonstrate the custom scrollbar</div>)}
            </div>
          </ScrollBar>
        </Card>

        <Card title={t('scrollbar.demo.color')} subtitle={t('scrollbar.demoDesc.color')}>
          <ScrollBar height={200} color="#1677ff" width={8}>
            <div style={{ border: '1px solid var(--sc-color-border, #e5e7eb)', 'border-radius': '8px', padding: '8px', 'font-size': '0.85rem', 'line-height': 2.5, color: '#6b7280' }}>
